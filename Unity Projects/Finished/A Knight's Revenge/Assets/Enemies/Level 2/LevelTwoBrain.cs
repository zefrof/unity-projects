...
public class LevelTwoBrain : MonoBehaviour {
	...
	void Update()
	{
		...
		{
			if(go.tag == "Enemy" && !enemies.Contains(go) && !go.GetComponent<BabySkullAI>().dead)
				enemies.Add (go);
			Debug.Log ("First if ran");
			if(go.tag == "Enemy" && go.GetComponent<BabySkullAI>().dead && enemies.Contains(go))
			{
				enemies.Remove(go);
				dead++;
