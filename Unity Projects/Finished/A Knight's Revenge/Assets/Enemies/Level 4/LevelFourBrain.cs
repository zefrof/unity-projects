...
public class LevelFourBrain : MonoBehaviour {
	...
	void Update()
	{
		...
		{
			if(go.tag == "Enemy" && !enemies.Contains(go) && !go.GetComponent<WaspAI>().dead)
				enemies.Add (go);
			Debug.Log ("First if ran");
			if(go.tag == "Enemy" && go.GetComponent<WaspAI>().dead && enemies.Contains(go))
			{
				enemies.Remove(go);
				dead++;
