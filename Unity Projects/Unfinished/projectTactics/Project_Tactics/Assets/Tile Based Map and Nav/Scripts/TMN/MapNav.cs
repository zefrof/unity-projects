...
public class MapNav : MonoBehaviour 
{
	...
	public TileNode[] GetPath(TileNode fromNode, TileNode toNode, TileNode.TileType validNodesLayer)
	{
		...
		while (openList.Count > 0)
		{
			...
			foreach (TileNode n in tn.nodeLinks)
			{
				...
				// check if there is a link switch between this and the target node and if it is on or off
				if (tn.linkOnOffSwitch != null)
				{
					if (tn.linkOnOffSwitch.LinkIsOn(n, validNodesLayer) == 0) continue;
				}
	...
	/// <summary>
	/// This will help setup the node links. It will turn off a link between two nodes of the height 
	/// difference between them are more than maxHeightDifference. Have a look TNELinksOnOffSwitch.cs
	/// mask is the mask applied when adding TNELinksOnOffSwitch </summary>
	public void SetupNodeLinkSwitches(float maxHeightDifference, TileNode.TileType mask)
	{
		if (nodesCache == null) return;
	...
	public void DeleteAllUnlinkedNodes()
	{
		...
		foreach (TileNode n in nodes)
		{
			...
			{
				if (nn == null) continue;
 				if (n.LinkIsOnWith(nn, (TileNode.TileType)(-1)))
				{
					isFine = true;
					break;
				}
