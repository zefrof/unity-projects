...
public class TileNode: MonoBehaviour
{
	...
	public List<TileNode> GetAllInRangeWithUnits(int radius)
	{
		List<TileNode> ret = new List<TileNode>();
		List<TileNode> helper = new List<TileNode>();
		_GetAllInRangeWithUnitsRecursive(radius, ref helper, ref ret);
		return ret;
	}

	private void _GetAllInRangeWithUnitsRecursive(int radius, ref List<TileNode> helper, ref List<TileNode> retNodes)
	{
		this._ShowNeighboursRecursive_Helper = radius;
		helper.Add(this);
		...
		foreach (TileNode node in this.nodeLinks)
		{
			...
			// check if a unit on the node
			if (node.units.Count > 0 && !retNodes.Contains(node))
			{
				retNodes.Add(node);
			}

			// test the next node(s)
			if (radius > 0)
			{
				node._GetAllInRangeWithUnitsRecursive(radius, ref helper, ref retNodes);
			}
		}
	}
		    ...
		    {				
		        if (n != null)
		        {
					...
					{	// check if neighbour's link with me is on
						if (nls.LinkIsOn(node, (TileType)(-1))==0) Gizmos.color = Color.red;
					}
