...
public class MapNavEditor : Editor
{
	...
	private static TileNode.TileType onOffMask = (TileNode.TileType)(-1);
	...
	public override void OnInspectorGUI()
	{
		...
		if (((MapNav)target).nodesCache != null)
		{
			...
			if (folds[7])
			{
				...
				onOffMask = (TileNode.TileType)EditorGUILayout.EnumMaskField("for Types", onOffMask);
				EditorGUILayout.BeginHorizontal();
				if (GUILayout.Button("Run Setup"))
				{
					((MapNav)target).SetupNodeLinkSwitches(maxHeightBeforeLinkBreak, onOffMask);
					EditorUtility.SetDirty(target);
				}
				...
				EditorGUILayout.EndHorizontal();
				EditorGUILayout.HelpBox("This tool helps automate setting up the links between nodes, depending on the height difference between the nodes. Enter a max height and links between nodes will be turned off if the height difference between them are higher.\n\nThe 'Delete' button allows for the auto-deletion of all nodes that have no active links.", MessageType.None);
